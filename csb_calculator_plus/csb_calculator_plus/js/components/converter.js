    ...
    switch (converterType) {
        ...
        case 'base':
            fieldsContainer.innerHTML = `
                ...
                <p>Result: <input class="result" id="base-result" readonly></p>
            `;
...
function convertBase() {
    ...
    if (baseFromValue === 'custom' && (isNaN(customBaseFrom) || customBaseFrom < 2 || customBaseFrom > 36)) {
        document.getElementById('base-result').value = 'Invalid custom base. Enter a value between 2 and 36.';
        return;
    }
...
function convertTime() {
    ...
    let formattedResult;
    if (unitTo === "second") {
        formattedResult = `${result.toFixed(2)} SECONDS`;
